...
---
En el siguiente chunk se cargarán las librerias necesarias:
...
trans %>%
  ...
  unique() %>% #escribe una nueva tabla con los campos que se le seleccione
  arrange(desc(total_transacciones)) %>% #Organiza la tabla en orden desc #%>% operador pipe
  kbl(caption = "Recreando mi tabla" ) %>%
  kable_classic(full_width = F, html_font = "Comic sans")
```
```{r}
trans %>%
  unique() %>%
  ...
  unique() %>% #escribe una nueva tabla con los campos que se le seleccione
  arrange(desc(total_transacciones)) %>% #Organiza la tabla en orden desc #%>% operador pipe
  kbl(caption = "Recreando mi tabla negrita" ) %>%
  kable_material_dark() %>%
